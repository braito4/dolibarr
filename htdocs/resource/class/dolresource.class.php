...
class Dolresource extends CommonObject
{
	...
	/** @var string link, requirement or assignment */
	public $relation_kind = 'link';
	...
	public function updateElementResource($user = null, int $notrigger = 0)
	{
		...
		$relationKind = $this->relation_kind;
		if (empty($relationKind)) {
			$relationKind = in_array($this->element_type, array('product', 'service'), true) ? 'requirement' : 'link';
		}
		if (!in_array($relationKind, array('link', 'requirement', 'assignment'), true)) {
			$this->error = 'Invalid resource relation kind';
			return -1;
		}
		if ($relationKind === 'requirement') {
			$allowedRequirementValues = array(
			...
			foreach ($allowedRequirementValues as $field => $values) {
				if (!in_array($this->{$field}, $values, true)) {
					$this->error = 'Invalid resource requirement field: '.$field;
					return -1;
				}
			}
			if ((float) $this->quantity_required <= 0 || (float) $this->users_per_service_unit < 0
				|| min((int) $this->duration_base, (int) $this->duration_per_unit, (int) $this->setup_duration, (int) $this->cleanup_duration) < 0) {
				$this->error = 'Invalid resource requirement quantity or duration';
				return -1;
			}
		}
		...
		$sql .= " relation_kind = '".$this->db->escape($relationKind)."',";
	 ...
	 * @param	string		$relation_kind		Relation semantic, inferred from element when empty
	 ...
	 */
	public function getElementResources(string $element, int $element_id, string $resource_type = '', string $relation_kind = '')
	{
		$relationKind = $relation_kind ?: (in_array($element, array('product', 'service'), true) ? 'requirement' : 'link');
		...
		if ($relationKind === 'link') {
			$sql .= " AND (relation_kind IS NULL OR relation_kind = 'link')";
		} else {
			$sql .= " AND relation_kind = '".$this->db->escape($relationKind)."'";
		}
		if ($resource_type) {
		...
		if ($resql) {
			...
			while ($i < $num) {
				...
				$resources[$i] = array(
					...
					'relation_kind' => $obj->relation_kind ?: $relationKind,
