...
class ResourceRequirementManager
{
	...
	public function getRequirements($elementType, $elementId)
	{
		...
		if (in_array($elementType, array('product', 'service'), true)) {
			// Service relations were historically stored with both aliases.
	...
	{
		return $this->buildTemporalPolicy($this->getRequirements($elementType, $elementId));
	}

	/**
	 * Combine already loaded requirements into an effective time-input policy.
	 *
	 * @param array<int,array<string,mixed>> $requirements Requirement rows
	 * @return array<string,mixed>
	 */
	public function buildTemporalPolicy(array $requirements)
	{
