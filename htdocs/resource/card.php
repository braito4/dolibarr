...
$status                 = GETPOSTISSET('status') ? GETPOSTINT('status') : null;
...
if (empty($reshook)) {
	...
	if ($action == 'add' && $permissiontoadd) {
		if (!$cancel) {
			...
			$allowedStatuses = array_keys(Dolresource::getStatusArray());
			if (!getDolGlobalInt('RESOURCE_ENABLE_UNKNOWN_AVAILABILITY')) {
				$allowedStatuses = array_diff($allowedStatuses, array(Dolresource::STATUS_UNKNOWN));
			}
			...
			if (!$error) {
				...
				$object->status                 = $status !== null ? $status : Dolresource::STATUS_FREE;
				...
				$result = !$error ? $object->create($user) : -1;
				if ($result > 0) {
					// Creation OK
	...
	if ($action == 'update' && !$cancel && $permissiontoadd) {
		...
		if (!$error) {
			...
			if ($res > 0) {
				...
				$object->url					= $url;
				$object->fk_code_type_resource  = $fk_code_type_resource;
				$object->status = $status !== null ? $status : $oldstatus;
...
if ($action == 'create' || $object->fetch($id, $ref) > 0) {
	...
	if ($action == 'create' || $action == 'edit') {
		...
		print '<form enctype="multipart/form-data" action="'.dol_escape_htmltag($formAction).'" method="POST" name="formresource">';
		...
		print '<td><input class="minwidth200" name="ref" value="'.dol_escape_htmltag($ref ?: $object->ref).'" autofocus="autofocus" spellcheck="false"></td></tr>';
		...
		print '<input type="tel" name="phone" id="phone" value="'.dol_escape_htmltag(GETPOSTISSET('phone') ? GETPOST('phone', 'alpha') : $object->phone).'"></td>';
		print '</tr>';
		...
		print '<input type="email" name="email" id="email" value="'.dol_escape_htmltag(GETPOSTISSET('email') ? GETPOST('email', 'alpha') : $object->email).'" spellcheck="false"></td>';
		print '</tr>';
		...
		print '<td>';
		print dol_escape_htmltag($object->type_label);
		print '</td>';
		print '</tr>';
