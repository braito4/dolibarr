...
(function ($) {
	...
	window.initResourceTemporalPolicy = function (config) {
		...
		function apply(policy) {
			...
			startFields.add(endFields).show().prop('disabled', false).removeClass('inputmandatory');
			$('.resource-time-start-label, .resource-time-end-label, #trlinefordates, #service_duration_area').show();
			$('#resource_temporal_policy').remove();
			$('.resource-time-seconds-added').remove();
			...
			function ensureSeconds(prefix) {
				if ($('[name="' + prefix + 'sec"]').length || !$('#' + prefix + 'min').length) return;
				var select = $('<select>', {id: prefix + 'sec', name: prefix + 'sec', class: 'flat resource-time-seconds resource-time-seconds-added'});
				for (var second = 0; second < 60; second++) select.append($('<option>', {value: second, text: String(second).padStart(2, '0')}));
				$('#' + prefix + 'min').after($('<span>', {class: 'resource-time-seconds resource-time-seconds-added', text: ':'})).after(select);
			}
