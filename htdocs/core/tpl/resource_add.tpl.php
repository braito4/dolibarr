...
$sqlResourceCapacityModes .= ' WHERE r.entity IN ('.getEntity('resource').')';
...
if ($element != 'product' && $element != 'service') {
	...
	$out .= '<input type="text" class="width75 right" id="users_per_service_unit" name="users_per_service_unit" value="'.dol_escape_htmltag(GETPOSTISSET('users_per_service_unit') ? GETPOST('users_per_service_unit', 'alpha') : '1').'" required>';
	$out .= '</div>';
...
if ($element == 'product' || $element == 'service') {
	$out .= '<script>jQuery(function(){var capacityModes='.json_encode($resourceCapacityModes).'; var role=jQuery("#resource_role"); var group=jQuery("input[name=requirement_group]"); var previousDefault="preferred_"+role.val(); function toggleVolumeAwareness(){var visible=capacityModes[jQuery("#fk_resource").val()] === "volume"; jQuery(".resource-volume-awareness").toggle(visible).find(":input").prop("disabled", !visible);} function updateDefaultGroup(){var nextDefault="preferred_"+role.val(); if (!group.val() || group.val() === previousDefault) {group.val(nextDefault);} previousDefault=nextDefault;} jQuery("#fk_resource").on("change", toggleVolumeAwareness); role.on("change", updateDefaultGroup); toggleVolumeAwareness();});</script>';
}
